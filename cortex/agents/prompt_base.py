...
COMMON_RULES = (
    ...
    "- Git workflows (commit, push, add+commit+push): chain in ONE call using &&. "
    "Example: git(args='add . && commit -m \"feat: description\" && push'). "
    "NEVER output git commands as text — call git() directly.\n"
    ...
    "- If the user ASKS something (a question, asks for advice, or asks which option to pick), "
    "ANSWER it first. Only reach for a tool for the parts that truly require an action "
    "(creating/fetching/changing something). Never run tools the question didn't ask for.\n"
